...
export default function Points()
{
  const navigate = useNavigate();
    ...
    useEffect(()=> {
        // axios.get("http://localhost:8080/valo-points-table").then((res) => {
        //   tempFunc(res.data);
        // });

        axios.get("https://esports-server.onrender.com/valo-points-table").then((res) => {
          tempFunc(res.data);
        });
    }, []);
    ...
    function MyVerticallyCenteredModal(props) {
        ...
        function handleSubmit(event)
        {
          ...
          // axios.post("http://localhost:8080/valo-edit-points", {tableItems, ID})
          // .catch(error => { console.log("Error sending Table Details: ", error) });

          axios.post("https://esports-server.onrender.com/valo-edit-points", {tableItems, ID})
          .catch(error => { console.log("Error sending Table Details: ", error) });
    
          setTableItems({played: "", wins: "", losses: "", tpoints: ""});
                      ...
                      <div className="App">
                          <form onSubmit={handleSubmit} method="post" autocomplete="off">
                            <div className="form-icon">
                              ...
                              <input className="input-form" type="text" name="played" placeholder=" Enter Matches Played" value={tableItems.played} onChange={handleChange} required />
                            </div>
                            <div className="form-icon">
                              <div className="icon">
                                <GoGraph size="15px" />
                              </div>
                              <input className="input-form" type="text" name="wins" placeholder=" Enter Matches Won" value={tableItems.wins} onChange={handleChange} required />
                            </div>
                            <div className="form-icon">
                              <div className="icon">
                                <GoGraph size="15px" />
                              </div>
                              <input className="input-form" type="text" name="losses" placeholder=" Enter Matches Lost" value={tableItems.losses} onChange={handleChange} required />
                            </div>
                            <div className="form-icon">
                              <div className="icon">
                                <GoGraph size="15px" />
                              </div>
                              <input className="input-form" type="text" name="tpoints" value={tableItems.tpoints} onChange={handleChange} placeholder="Enter Total Points" required />
                            </div>
                            <div>
    ...
    {
      var gamename = "VALORANT", direct = "/coming-soon";
    ...
    return (
      ...
      <div className="points">
          ...
          {status && <button className="start-btn" onClick={handleClick2}>Tourni over</button>}
            <table>
                <thead>
                    <tr >
